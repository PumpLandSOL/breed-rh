// hong.fun hype video — deterministic render: seek each frame, screenshot, encode locked 30fps (zero dropped frames).
'use strict';
...
const W = 1280, H = 720, FPS = 30, DUR = 12, PORT = 9519;
const OUT = path.join(__dirname, '..', 'brand', 'breed-studfees-12s.mp4');
const FRAMES = path.join(__dirname, 'stud-frames');
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));
(async () => {
  fs.rmSync(FRAMES, { recursive: true, force: true }); fs.mkdirSync(FRAMES);
  const url = 'file:///' + path.join(__dirname, 'stud-video.html').split(path.sep).join('/');
